...
echo "$($PSQL "SELECT DISTINCT t.name FROM teams t INNER JOIN games g ON t.team_id = g.winner_id OR t.team_id = g.opponent_id WHERE g.year = 2014 AND g.round = 'Eighth-Final'")"
...
echo "$($PSQL "SELECT DISTINCT g.year, t.name FROM teams t INNER JOIN games g ON t.team_id = g.winner_id WHERE g.round = 'Final'")"
